...
const target = document.querySelector('.products-container');

const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
        if (!entry.isIntersecting) {
        // Unload or hide when not visible
        target.innerHTML = ''; // or target.remove();
        } else {
        // Reload content if needed
        target.innerHTML = '<p>This section will unload when not visible.</p>';
        }
    });
});
